  ...
  const render = async () => {
    ...
    root.innerHTML = `
      ...
      <div class="mb-8">
        ...
        <p class="text-sm text-[#75777E] mt-1">إضافة وتعديل الشركات والعلامات التجارية.</p>
      </div>
    ...
    // Bind Edit/Delete buttons -- scoped to this table's own tbody (kept
    // scoped, not switched to a bare root.querySelectorAll(), even though
    // this page only has one table now that the wholesale-section CRUD has
    // moved to wholesale-sections-crud.js -- this file had a cross-wiring
    // bug from exactly that unscoped pattern when it DID have two tables;
    // no reason to reopen that risk if a second table is ever added here again).
