...
export function updateQuantity(variantId,newQuantity){const items=getCartItems();const item=items.find(entry=>entry.variant_id===variantId);if(!item)return items;if(Number(newQuantity)<=0)return removeFromCart(variantId);item.quantity=Math.max(1,Math.floor(Number(newQuantity)||1));return save(items)}
export function removeFromCart(variantId){return save(getCartItems().filter(item=>item.variant_id!==variantId))}
